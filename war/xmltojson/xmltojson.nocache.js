function xmltojson(){
  ...
  $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'begin'});
  ...
  function maybeStartModule(){
    if (scriptsDone && loadDone) {
      var iframe = $doc_0.getElementById('xmltojson');
      ...
      xmltojson = null;
      frameWnd.gwtOnLoad(onLoadErrorFunc, 'xmltojson', base, softPermutationId);
      $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'end'});
    }
  }
  ...
  function computeScriptBase(){
    ...
    function tryNocacheJsTag(){
      ...
      for (var i = 0; i < scriptTags.length; ++i) {
        if (scriptTags[i].src.indexOf('xmltojson.nocache.js') != -1) {
    ...
    function tryMarkerScript(){
      ...
      if (typeof isBodyLoaded == 'undefined' || !isBodyLoaded()) {
        var markerId = '__gwt_marker_xmltojson';
        var markerScript;
  ...
  function processMetas(){
    ...
    for (var i = 0, n = metas.length; i < n; ++i) {
      ...
      if (name_0) {
        name_0 = name_0.replace('xmltojson::', '');
        if (name_0.indexOf('::') >= 0) {
          continue;
        }
  ...
  function maybeInjectFrame(){
    if (!frameInjected) {
      ...
      $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'moduleStartup', millis:(new Date).getTime(), type:'moduleRequested'});
  ...
  xmltojson.onScriptLoad = function(){
    if (frameInjected) {
      loadDone = true;
      maybeStartModule();
    }
  ...
  ;
  xmltojson.onInjectionDone = function(){
    scriptsDone = true;
    $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'end'});
    maybeStartModule();
  }
  ;
  processMetas();
  ...
  var initialHtml;
  if (isHostedMode()) {
    if ($wnd_0.external && ($wnd_0.external.initModule && $wnd_0.external.initModule('xmltojson'))) {
      $wnd_0.location.reload();
      return;
    }
    initialHtml = 'hosted.html?xmltojson';
    strongName = '';
  }
  $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'selectingPermutation'});
  if (!isHostedMode()) {
    try {
      unflattenKeylistIntoAnswers(['ie6'], '209C6A33E3F49BC16C2154090686FC16');
      unflattenKeylistIntoAnswers(['ie8'], '32C3667BE143F109237E488FA7DB72A0');
      unflattenKeylistIntoAnswers(['ie9'], '4085B25293C924D872AF5611D3EE5D49');
      unflattenKeylistIntoAnswers(['opera'], '697FB671C89365DFA51A55ECBE98B5C4');
      unflattenKeylistIntoAnswers(['safari'], '70C2176F903C5CA682937871D4C637B5');
      unflattenKeylistIntoAnswers(['gecko1_8'], 'D6846EA023D30187DE35EF9D834D3498');
  ...
  , 50);
  $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'bootstrap', millis:(new Date).getTime(), type:'end'});
  $stats && $stats({moduleName:'xmltojson', sessionId:$sessionId_0, subSystem:'startup', evtGroup:'loadExternalRefs', millis:(new Date).getTime(), type:'begin'});
  ...
  $doc_0.write('<script defer="defer">xmltojson.onInjectionDone(\'xmltojson\')<\/script>');
}
